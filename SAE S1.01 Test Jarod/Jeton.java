...
public class Jeton {
    ...
    public static double[][] cases = new double[NCASES][NCASES];
    ...
    static final double RADIUS = SIZE/NLIGNES; //Taille des cercles StdDraw
  ...
  */
  public static boolean jouer(String couleur, int val, int pos){
    double xb=-SIZE+(SIZE*0.15);
    double yb=SIZE-(SIZE*0.15);
    double xr=SIZE-(SIZE*0.15);
    double yr=SIZE-(SIZE*0.15);
        String nombre;
        ...
        String value = String.valueOf(val);
           if (pos>=0 && pos <NCASES && state[pos].equals("___")){
               if (couleur.equals("B")){
                   ...
                   if (val<10) {
                     StdDraw.filledCircle(cases[pos][pos],cases[pos][pos],RADIUS);
                     StdDraw.setPenColor(StdDraw.WHITE);
                     StdDraw.text(cases[pos][pos],cases[pos][pos],value);
...
*/
    ...
    public static void afficheJeton(){
      ...
      //int iy;
      double x;
      double y=SIZE+(0.2*RADIUS);
      String id;
      for (int i = 1 ; i <= NLIGNES ; i++ ) {  //parcours les lignes
        y-=(SIZE/5)+(0.5*RADIUS);
         for ( int j = 1 ; j <= i ; j++) { //parcours les cases par lignes
           x=((RADIUS*2)+(0.1*RADIUS))*(j-1)-(RADIUS*i);
           StdDraw.circle(x+RADIUS,y,RADIUS);
           id = String.valueOf(idcase);
           StdDraw.text(x+(RADIUS+(RADIUS*0.2)),y-(RADIUS*0.4),id);
           //ix=(int) Math.round(x);
           //iy=(int) Math.round(y);
           //cases[ix][iy]=ix + iy;
           cases[j][i]=x + y;
